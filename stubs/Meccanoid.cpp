...
ServoAdapter & ServoAdapter :: setColor(byte color) {
	...
	case 1: // BLUE
		return setColor(0,0,1);
	
	case 2: // GREEN
		return setColor(0,1,0);

	case 3: // CYAN
		return setColor(0,1,1);

	case 4: // RED
		return setColor(1,0,0);

	case 5: // PURPLE
		return setColor(1,0,1);

	case 6: // YELLOW
		return setColor(1,1,0);
...
byte Chain::getType(int pos){
	return modules[pos].type;
}
MeccanoServo* Chain :: getServo(int id) {
	return new ServoAdapter(modules[constrain(id, 0, MAX_CHAIN - 1)], *this);
}

MeccanoLed* Chain :: getLed(int id) {
	return new LedAdapter(modules[constrain(id, 0, MAX_CHAIN - 1)], *this);
}
